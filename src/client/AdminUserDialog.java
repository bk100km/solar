...
public class AdminUserDialog extends JDialog implements ActionListener{
	...
	public AdminUserDialog(AdminUserFrame auf) {
		super(auf, "회원정보 수정", true);
		this.auf = auf;
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == update) {
			int row = auf.table.getSelectedRow();
				if (dao.updateByAdmin(this) > 0) {
					JOptionPane.showMessageDialog(this, "수정 완료");
					dispose();
					dao.selectAllByAdmin(auf.dTable);
					if (auf.dTable.getRowCount() > 0)
						auf.table.setRowSelectionInterval(row, row);
				} else {
